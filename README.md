...
Instruction for building the PostRec recommender system from source code and executing experiments
</h3>
<ul>
...
<ul>
<li>Prepare Data</li>
<li>The mentioned neural network models are in PostRec/post_rec/models. </li>
<li>Run the scripts in PostRec/test/db_test/ folder to prepare training data. </li>
<li>Run the scripts in PostRec/test/retriver_test/ folder to build the model mentioned in PostRec.</li>
</ul>
...
<li>
Make the dirs listed in PostRec/post_rec/__init__.py.
</li>
</ul>
...
Store the pretrained model weight and auxiliary data of BERT model to the dirs BertBasePath or BertLargePath mentioned in PostRec/post_rec/__init__.py.
</strong></p>
...
<p><strong>Data Analysis and Link Analysis</strong></p>
  <ul>
  <li>
  Run PostRec/test/associationAlg_test/seedSearchForTags.py to analyze the AI related tags and using association mining to find all required posts data.
  </li>
  <li>
  Run PostRec/test/graphLinke_test/build_link_path.py to build the posts link graph. If you have a spark cluster, you can boost the computaion space via running PostRec/test/graphLinke_test/spark-graph.py; or you can run PostRec/test/graphLinke_test/extract_link_semi_path.py to build an incomplete graph for quick test.
  </li>
  <li>
  Exract link distance posts pairs: run PostRec/test/graphLinke_test/build_label_pair.py to generate "link distance + posts ids(1+2)" data record, which is later used to generate inference task data.
  </li>
  </ul>


<p><strong>Training Data for PostRec</strong></p>
  <ul>
  <li>
  Run PostRec/test/db_test/gen_corpus+_inference.py and push the generated corpus to mongodb cluster.
  </li>
  <li>
  Run PostRec/test/db_test/gen_samples.py with task parameter as 'inference' to sample training and validating data.
  </li>
  <li>
  Preprocess the generated samples by running PostRec/test/tokenizer_test/tokenize_corpus.py.
  </li>
  </ul>
...
<p><strong>Build Local Knowledge Base</strong></p>
  <ul>
  <li>
  Run PostRec/test/db_test/buildQAIndexer.py firstly to gather all answers to each question.
  </li>
  <li>
  Run PostRec/test/db_test/gen_kwnowledge_unit.py to generate knowledge unit data used by KnowAlpha.
  </li>
  <li>
...
<ul>
<li>
The document search engine is in PostRec/post_rec/retrievers/SearchEngine folder.
</li>
<li>
...
<ul>
<li>
Run PostRec/test/retriever_test/build_linkprediction_model.py to train the Knowledge Inference Net.
</li>
<li>
...
<p><strong>Evaluate the PostRec</strong></p>
<ul>
<li>
Sample 2000 solved questions via runining "PostRec/test/db_test/gen_samples.py --maxSize 2000 --task inference" to generate the test samples. 
</li>
<li>
Run the PostRec/test/retriever_test/run_model.sh --do_eval to predict the link distance results directly, which is used to measure model performance on the test samples.
</li>
<li>
Run the PostRec/test/retriever_test/interactive.py with input stream re-directed to a file containing post ids of test samples, which is the evaluation of KnowAlpha.
</li>
<li>
1)Use the sklearn metrics toolkit to evaluate the model performance of Inference Net; 2) Refer to https://github.com/microsoft/recommenders for evaluation of the retrieved results of PostRec. 
</li>
<li>
...
author={#}, 
title={Finding Useful Posts for Programmers through Semantic Understanding
},
year={2019},
url={https://github.com/zhangzhenyu13/PostRec} 
}
</pre></code>
  </body>
</html>

...
from post_rec.tokenizers import BertTokenizer,OpenAIGPTTokenizer,TransfoXLTokenizer
...
modelPath={
    "bert": post_rec.BertBasePath,
    "openAI": post_rec.openAIGPTPath
}
...
def extractBert():
    ...
    #for i in range(10):
    #    print(weight[i])
    with open(post_rec.Bert768 + "embeddings.txt", "w") as f:
        vec_strs=[]
...
def extractOpenAI():
    ...
    with open(post_rec.openAI768 + "embeddings.txt", "w") as f:
        for i  in range(len(weight)):
            vec=weight[i]
            vec_str=list(map(lambda x :str(x),vec))
    ...
    with open(post_rec.openAI768 + "vocab.txt", "w") as f:
        for i in range(len(weight)):
            token=tokenizer.decoder[i]
            f.write(token+"\n")

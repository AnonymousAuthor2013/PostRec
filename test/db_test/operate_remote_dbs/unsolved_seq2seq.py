...
import post_rec
from post_rec.tokenizers import BertTokenizer
...
def init(questionsData_G,answersData_G,copy=True):
    ...
    tokenizer=BertTokenizer(post_rec.ModelPath + "knowledgeComprehension/vocab.txt", never_split=never_split)
...
def generateContextAnswerCorpusParallel(distanceData,questionsDataGlobal,answersDataGlobal):
    ...
    with open(post_rec.DataPath + "Corpus/unsolved", "a") as f:
...
def main():
    ...
    distance_file= post_rec.DataPath + "linkData/" + dbName.lower() + '-2graph.json'
    distance_data=[]
    q_ids_set=set()

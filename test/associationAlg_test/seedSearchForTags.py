...
from post_rec.Utility.CorrelationAnalysis import Apriori,stepAprioriSearch,mineFPTree,FPTree
import numpy as np
import post_rec
import json,pickle
...
def getFrequentItems(load=True):
    if not load:
        ...
        with open(post_rec.DataPath + "frequentItems.pickle", "wb") as f:
            pickle.dump(frequentItems,f)
    else:
        with open(post_rec.DataPath + "frequentItems.pickle", "rb") as f:
